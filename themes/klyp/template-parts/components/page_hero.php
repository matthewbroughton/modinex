...
<?php if ($section_show == true) : ?>
    <section id="<?= $section_id; ?>" class="<?= $section_class; ?> relative overflow-hidden min-h-[24rem] lg:min-h-[36rem] flex items-center -mb-px product-cat__banner py-12 sm:py-0">
        <?php
                ...
                <?php if ($feature_cta_enabled == true) : ?>
                    <div class="flex flex-col sm:flex-row gap-4 mt-8">
                        <?php if ($feature_cta_url_1) : ?>
                            <a class="border border-white bg-white text-black flex justify-between items-center gap-2 rounded-full py-2 px-4 lg:min-w-[16.875rem] transition hover:bg-sage hover:text-white hover:border-sage" href="<?= $feature_cta_url_1['url']; ?>" target="<?= $feature_cta_url_1['target']; ?>"><span><?= $feature_cta_url_1['title']; ?></span> <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" class="w-5 h-5"><path fill-rule="evenodd" d="M3 10a.75.75 0 01.75-.75h10.638L10.23 5.29a.75.75 0 111.04-1.08l5.5 5.25a.75.75 0 010 1.08l-5.5 5.25a.75.75 0 11-1.04-1.08l4.158-3.96H3.75A.75.75 0 013 10z" clip-rule="evenodd" /></svg></a>
                        <?php endif; ?>
                        <?php if ($feature_cta_url_2) : ?>
                            <a class="border border-white flex justify-between items-center gap-2 rounded-full text-white py-2 px-4 lg:min-w-[16.875rem] transition hover:bg-sage hover:text-white hover:border-sage" href="<?= $feature_cta_url_2['url']; ?>" target="<?= $feature_cta_url_2['target']; ?>"><span><?= $feature_cta_url_2['title']; ?></span> <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" class="w-5 h-5"><path fill-rule="evenodd" d="M3 10a.75.75 0 01.75-.75h10.638L10.23 5.29a.75.75 0 111.04-1.08l5.5 5.25a.75.75 0 010 1.08l-5.5 5.25a.75.75 0 11-1.04-1.08l4.158-3.96H3.75A.75.75 0 013 10z" clip-rule="evenodd" /></svg></a>
                        <?php endif; ?>
                    </div>
                <?php endif; ?>
            </div>
